...
/24や/16のような表記から、使えるIPアドレスの範囲を瞬時に計算できるようになるには、パターン認識のトレーニングが必要です。

AIを使った練習法として効果的なのは、次のループを繰り返すことです。

```text
私はCCNA試験の準備中です。
以下の形式でサブネット計算の練習問題を10問出してください。
- 問題: ネットワークアドレスとサブネットマスクを与える
- 回答: ネットワークアドレス、ブロードキャストアドレス、
        使用可能ホスト数を答えさせる

問題を出し終えたら、私が回答した後に採点と解説をしてください。
難易度は「/24〜/28のCIDR表記」に絞ってください。
```

市販の問題集には問題数の上限がありますが、AIは同じパターンで無限に問題を生成できます。

<strong>1日15分のAI練習ドリルを2週間続けると、サブネット計算の感覚が体に染み込みます</strong>。
