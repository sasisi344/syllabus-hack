...
relatedPosts:
  ...
  - /theory/ui-ux-accessibility-universal-design/
noteUrl: ''
draft: false
lastmod: 2026-09-15
metadata:
  description: 'ITパスポート20時間コース第5章。CPU・メモリなどのハードウェア、SaaS/PaaS/IaaSのクラウド、UI/UXなどの情報デザインを精選44語で押さえる。'
---

import ChapterQuiz from '~/apps/course-quiz/ChapterQuiz';
import questions from '~/data/quiz/it-passport/course-ch5.json';
...
> 深掘り: [UI・UX・アクセシビリティ・ユニバーサルデザインとは？情報デザイン4用語を1ページで整理](/theory/ui-ux-accessibility-universal-design/)
...
```
あなたはITパスポート試験の講師です。「PC・システムの基礎」の範囲について、以下の用語を初学者向けに**関連づけながら**説明してください（単体の丸暗記より、用語同士のつながりを理解したほうが記憶に定着しやすく、試験本番で紛らわしい選択肢を見分ける力もつくためです）。用語同士の違いが分かる**比較表**を最後に付けてください。
...
<ChapterQuiz client:load questions={questions} examId="ip" mode="chapter" chapterOrder={5} />

## この章のまとめ

この章では、CPU・メモリなどハードウェアの役割分担、SaaS・PaaS・IaaSといったクラウドの分類、そしてUI/UXなど情報デザインの基礎を学びました。

次章では、そのPCやシステムが世界とどうつながるかという<strong>ネットワーク</strong>と、情報を整理して保存する<strong>データベース</strong>の仕組みを学びます。→ [第6章 ネットワークとデータベース](/course/ip/06-network-database/)
