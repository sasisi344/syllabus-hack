...
lastmod: 2026-09-15
metadata:
  description: 'ITパスポート20時間コース第6章。LAN/WAN・DNS・TCP/UDPなどのネットワークと、正規化・排他制御などのデータベースを精選30語で押さえる。'
---

import ChapterQuiz from '~/apps/course-quiz/ChapterQuiz';
import questions from '~/data/quiz/it-passport/course-ch6.json';
...
> 深掘り: [OSI参照モデル・TCP/IP・DNS・正規化とは？](/theory/network-database-fundamentals/)
...
```
あなたはITパスポート試験の講師です。「ネットワークとデータベース」の範囲について、以下の用語を初学者向けに**関連づけながら**説明してください（単体の丸暗記より、用語同士のつながりを理解したほうが記憶に定着しやすく、試験本番で紛らわしい選択肢を見分ける力もつくためです）。用語同士の違いが分かる**比較表**を最後に付けてください。
...
<ChapterQuiz client:load questions={questions} examId="ip" mode="chapter" chapterOrder={6} />

## この章のまとめ

この章では、LAN・WANやTCP/IPといったネットワークの基本構造、そして正規化や排他制御といったデータベースの設計・操作の考え方を押さえました。

次章では、そうして蓄積されたデータをどう管理・分析するかという<strong>データマネジメント</strong>と、いま最も注目度の高い<strong>AI</strong>の技術・活用法を学びます。→ [第7章 データとAI](/course/ip/07-data-ai/)
