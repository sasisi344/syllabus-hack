      ...
      {showConfig && (
        <div className="mb-8 p-4 bg-slate-800 rounded-lg border border-slate-700 animate-in slide-in-from-top-4 duration-300">
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-xs font-bold text-slate-400 mb-1 uppercase">My Module (背景・経験・武器)</label>
              <textarea 
                value={myModule}
      ...
      {!selectedTheme ? (
        <div className="text-center py-20 border-2 border-dashed border-slate-700 rounded-2xl bg-slate-800/50">
          <div className="mb-6">
            ...
            <p className="text-slate-400 text-sm max-w-xs mx-auto">テーマ、論点、シチュエーションがランダムに生成されます。</p>
          </div>
          <button
            onClick={drawGacha}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 animate-in fade-in zoom-in duration-500">
          ...
          <div className="space-y-6">
            <div className={`transition-all duration-700 ${isFlipped ? 'rotate-y-0 opacity-100' : 'rotate-y-12 opacity-0'}`}>
              <div className="bg-slate-800 p-6 rounded-2xl border border-slate-700 shadow-inner relative overflow-hidden">
                ...
                <div className="flex items-center gap-2 mb-4">
                  <span className="px-2 py-0.5 rounded bg-blue-500 text-[10px] font-bold text-white uppercase">{selectedTheme.category}</span>
                  <span className="text-slate-500 text-xs font-mono">ID: {Math.random().toString(36).substring(7).toUpperCase()}</span>
                </div>
...
・プロンプト管理の徹底..."
                ...
                >
                  {isEvaluating ? (
                    ...
                    <>
                      <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 11.5a8.38 8.38 0 0 1-.9 3.8 8.5 8.5 0 1 1-7.6-10.4 8.38 8.38 0 0 1 3.9.9"/><polyline points="21 3 12 12"/><polyline points="21 8 21 3 16 3"/></svg>
                      構成案をAIに提出
                    </>
                  )}
                </button>
              ...
              <div className="flex-1 space-y-6 animate-in slide-in-from-bottom-8 duration-500">
                <div className="p-6 bg-slate-800 rounded-2xl border border-slate-700 relative overflow-hidden">
                  <div className={`absolute top-4 right-6 text-6xl font-black italic ${
                    evaluation.rank === 'A' ? 'text-green-500/20' : evaluation.rank === 'B' ? 'text-amber-500/20' : 'text-red-500/20'
                  }`}>
                    {evaluation.rank}
                  </div>
                  <h4 className="text-xl font-bold mb-6 flex items-center gap-2">
                    <span className={`w-8 h-8 rounded-full flex items-center justify-center text-sm ${
                      evaluation.rank === 'A' ? 'bg-green-600' : evaluation.rank === 'B' ? 'bg-amber-600' : 'bg-red-600'
                    }`}>
                      {evaluation.rank}
                    </span>
                    AI総合判定
                      ...
                      <div key={key} className="bg-slate-900/50 p-3 rounded-xl border border-slate-700">
                        <div className="flex justify-between items-center mb-1">
                          <span className="text-[10px] text-slate-500 uppercase font-bold">
                            {key === 'consistency' ? '一貫性' : key === 'constraint' ? '制約対応' : key === 'specificity' ? '具体性' : '論理'}
                          </span>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="p-4 bg-indigo-600/10 rounded-xl border border-indigo-600/20">
                      ...
                      <div className="text-sm text-indigo-200/90 leading-relaxed italic">
                        {evaluation.advice}
                      </div>
                    </div>
                  </div>
