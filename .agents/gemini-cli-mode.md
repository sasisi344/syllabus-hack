...
| `.workspace/.task/` | **司令塔（Command）** | タスクの要件定義（Requirement）。TODOの状態管理。 |
| `.workspace/draft/` | **下書き（Draft）** | 記事やコードのプロトタイプ作成場所。 |
...
1. **要件確認**: `.workspace/.task/` 内の該当TODOを確認。
...
1. **要件確認**: `.workspace/.task/` 内の `requirement-*.md` を確認。
...
2. **ルール適用**: `.workspace/.task/SNS-post-schedule/cli-snspost-schedule.md` をロード。
3. **出力**: `.workspace/.task/SNS-post-schedule/{YYYY-MM-schedule}/` に書き出し。
