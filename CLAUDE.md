...
```
1. **bold** → <strong> 全変換（src/data/post/ 内では Markdown bold 禁止）
...
- **強調表記**: 本番（`src/data/post/`内）では `<strong>強調内容</strong>` を使用。`**bold**` 記法は禁止
- **ヘッダー**: 見出しに `「」` 括弧を使わない。番号付け（`## 1. Step`）も禁止
- **bold密度**: 1段落あたり1〜2個が上限。同じキーワードを同記事内で重複してboldしない
- **bold前後スペース**: `...は <strong>ITパスポート</strong> です` のように前後に半角スペース
