...
<!-- island: quizRef=course-ch6。UI仕様はip-course-curriculum.md §3-5参照 -->

1. 通信の信頼性を重視し、パケットの到達確認や再送を行うプロトコルはどれか。
   - ア: UDP
   - イ: TCP
   - ウ: DHCP
   - エ: NAT
   - **正解**: イ
2. ドメイン名をIPアドレスに変換する仕組みはどれか。
   - ア: NAT
   - イ: DHCP
   - ウ: DNS
   - エ: ファイアウォール
   - **正解**: ウ
3. データベースの正規化を行う主な目的として正しいものはどれか。
   - ア: 処理速度を上げるため
   - イ: データの重複や矛盾を排除し、更新時の不整合を防ぐため
   - ウ: 暗号化を強化するため
   - エ: バックアップを自動化するため
   - **正解**: イ
4. 複数の処理が同時にデータを更新しようとして、お互いに相手の処理完了を待ち続け動けなくなる状態はどれか。
   - ア: コミット
   - イ: ロールバック
   - ウ: デッドロック
   - エ: 排他制御
