...
> 2026-07-10 更新（index-articles.js 集計、総数365件。リダイレクト統合済み draft 18件は `.workspace/draft/archive/` へ移動済み・計上外）

| カテゴリ | 件数 (6/9) | 件数 (7/10) | 評価 |
|--------|----:|----:|------|
| method | 107 | 114 | 過多・分散。ハブ記事で整理済みだが個別記事との紐付けが弱い |
| theory | 86 | 92 | ITパスポートレベルに偏り。FE/AP理論が薄い |
| trend | 61 | 81 | 試験情報とキーワード解説が混在（theory性格の記事は knowledge.type: theory で明示済み） |
| career | 44 | 55 | ペルソナ別の深掘りが不足 |
| app | 23 | 23 | 強み。IPA系は充実、他資格はゼロ |
| Uncategorized | 6 | 0 | ✅ 解消済み |

### 既存ハブ記事

ITP・FE・AP・SG・高度試験・AWS+CCNA・FP2 の7クラスターはハブ記事あり。
MOS・DS検定・G検定・簿記・宅建・電験三種・危険物・ビル管理・土木施工管理はハブあり（下表参照）。E資格・TOEICはほぼ未着手。

### 波3クラスター充足率（2026-07-10・site-check0710 WP04 D-2で追加）

| examId | クラスター | Hub | Method | Career | Theory | App | 評価 |
|---|---|:---:|:---:|:---:|:---:|:---:|---|
| `denken` | 電験三種 | ◎ | × | × | × | × | Hub単独。1000時間の壁に対する属性別（文系・社会人）ニッチKW未開拓 |
| `kiken-butsu` | 危険物取扱者（乙4） | ◎ | × | × | × | × | Hub単独。年180万人規模だが短期集中・社会人角度のスポーク未着手 |
| `biru-kanri` | ビル管理クラスター | ◎ | × | × | × | × | Hub単独。3資格（ビル管理士・マンション管理士・エネルギー管理士）で受験資格が異なるため、サブ資格別スポーク分離が急務 |
| `doboku-sekou` | 土木施工管理技士 | ◎ | × | × | × | × | Hub単独。1級・2級の未経験者ルート（学科のみ挑戦）が個別記事化されていない |

> 波3は4クラスターすべてがHub単独状態。KW-DB新設（`.workspace/data-set/cert-keyword-db/`）は2026-07-10に完了。次は各クラスター最低1本のスポーク追加が優先度SS。
