...
- [x] フッターの3カラム再編 → 2026-09-15完了（ユーザー指示）。①カテゴリリンク ②運営者簡易プロフィール＋利用規約・プライバシーポリシー ③300×250広告枠プレースホルダー（広告コードは未実装、後日差し替え）。詳細は[`site-structure-pillars.md`](site-structure-pillars.md) §7参照
- [ ] トップページの3本柱カードセクション・副次セクションの実装はまだ（Hero直下の20時間コースCTAは既存のまま活用中）
