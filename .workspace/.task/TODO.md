...
## 6. 【優先度：低・ストック企画】日商簿記「AIで代替されるのになぜ売れる？」考察記事＋簿記20時間学習案

> 背景（2026-09-26 ユーザー観測）: 書籍の売れ筋ランキングに日商簿記3級のテキストが多数並ぶ一方、2級・1級のテキストは少ない。「簿記はAIで代替される」と言われながら資格需要が根強い理由を考察し、記事化する。着手時期は未定（§4と同様、次の企画ネタに困った時に着手）。
> **関連ナレッジ**: [`.workspace/data-set/cert-keyword-db/boki-kw-db.md`](../data-set/cert-keyword-db/boki-kw-db.md)（着手時は `.agents/kw_pattern_research.md` 経由でKWを再実査すること）

- [ ] **考察テーマA**: 簿記はAI（会計ソフト・自動仕訳）で代替されるのに、なぜ資格として必要とされ続けるのか
  - 仮説: 入力作業は代替されても「数字を読む力」（決算書の理解・AI出力の検証）は代替されない／事務職・経理の採用で最低ラインの証明として機能／ビジネス教養・リスキリング需要（会計リテラシー）
- [ ] **考察テーマB**: 売れ筋が3級に偏り、2級・1級が少ないのはなぜか
  - 仮説: 3級は学生・社会人の「教養枠」で母数が桁違い（受験者数データで裏付け要）／2級以上は実務職志望者に絞られ、講座・通信教育（資格スクエア等）へ流れる／1級は専門学校・税理士受験ルートで独学書籍市場が小さい／ネット試験化で3級の受験ハードルが低下
- [ ] **簿記の20時間学習案**: 3級範囲を「20時間で体系的に学ぶ」構成案（§4の非IPA系20hoursラインとの統合可否も検討。会計・財務の基礎は§4の追加候補にも挙がっている）
- [ ] 着手時: 受験者数・合格率（日商公式統計）、書籍ランキングの実データを取得して仮説を検証 → カテゴリ（trend/career）とフレームワーク（QUEST想定）を確定
