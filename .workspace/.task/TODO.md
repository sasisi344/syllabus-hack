...
- [x] DS検定: データサイエンティストの資格試験になる。文系は就活市場でかなり不利になっているので、最低限でもDS資格なりスキルは持っておくべきとの指摘がある。「DS検定とG検定を文系が取得するにはどうしたらいいか？」を体系的に説明する記事を作成。→ `src/data/post/trend/ds-kentei/` に公開済み（2026-05-06）
