...
- [x] **秘書検定 × マルチワーク・フリーランス活用記事**（2026-06-03）
  - `career/secretarial-exam-multiwork-freelance` 作成
  - 秘書検定2級 + IT資格（SG/ITパスポート）の3資格組み合わせ戦略 / AI補助ツール活用 / FAQ4問
- [x] **タグページのインデックス解禁**（2026-06-03）
  - `config.yaml`: `tag.robots.index: true` に変更
  - `[tag]/[...page].astro`: `page.total >= 10` の条件でインデックス制御を追加
  - 10記事以上のタグ（ITパスポート130件・生成AI等）はインデックス、薄いタグはnoindex
  - タイトルを日本語化・記事数表示を追加
- [x] **地域特性に合わせたキャリア戦略リライト**（2026-06-03）
  - `career/regional-salary-qualification-correlation` 作成
  - 都道府県別IT年収表（厚生労働省統計引用）・資格保有者との待遇差・地方活用3戦略・FAQ3問
- [x] **ユーザーの地域を判定しハックをサジェストする機能 Phase A**（2026-06-03）
  - `src/apps/region-suggest/RegionWidget.tsx` 作成（47都道府県選択 → LocalStorage保存 → 地域別記事カード表示）
  - 9都道府県（北海道・宮城・東京・神奈川・静岡・愛知・大阪・広島・福岡）に専用記事マッピング
  - `SinglePost.astro` に統合：`-hub` 系スラグページの記事末尾に自動表示
  - Phase B（IPジオロケーション API による自動推定）は将来タスクとして保留
