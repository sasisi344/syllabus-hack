...
**W30追補（2026-07-20）**: `01_diary/weekly/2026/07-W30.md`（別ボルト・データドリブン週報）で `/method/cbt-2026-syllabus-complete-guide/`（表示98・クリック1・CTR1.02%）が異常値として再検出された。前回セッションの未コミット編集がタイトルから「基本情報」「シラバス」を落とす方向で進行中だったが、同レポートの4-Box分析で「基本情報技術者試験 シラバス2026」クエリ自体はCTR4.0%→8.8%・順位5.8→6.6と改善中で推奨アクションは「上位化施策（被リンク等）」だったため、**タイトル変更ではなく内部リンク強化で対応**（`method/fe-hub`・`trend/ipa-exam-trends-2026`から新規リンク追加）。あわせて`method/ap-hub`に「午後記述式」FAQを追加（新規クエリ対応）。両方`pnpm build`1504ページ・エラーなし確認済み、コミット済み（push待ち）。効果判定はW31以降のGSCで実施。
