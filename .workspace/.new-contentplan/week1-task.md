...
- [ ] **（新規追加・2026-09-07）** 【ユーザー作業】noteアカウントのハンドル確保のみ今月実施（ブランド名の先行押さえ。マガジン設計・価格・本文制作・メールゲートは来月着手のまま据え置き）
  - **自己紹介文（採用確定・2026-09-07）**: 「シラバスハック公式note。資格試験の分厚い『合格テキスト』を生成AI時代の学び方で再構築中。まずはITパスポートから。無料の入口編（20時間学習法）は本体サイトへ」
  - ハンドルは未確定（候補: `syllabushack` / `sh_text` / `goukaku_text_lab` / `sh_goukaku`。SNSと同一の`syllabushack`系を推奨）
