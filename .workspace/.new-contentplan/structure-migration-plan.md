...
## 10. コンテンツ3本柱への再編（2026-09-09・ユーザー起点、設計提案）

> ユーザー要望: 新しいコンテンツの柱を「20時間学習」「生成AIと学ぶ」「資格試験の教材」の3本に据え、他に「最新情報のフォロー」「コラム」を副次コンテンツとして持たせたい。**本節はその設計提案**であり、§9-2のナビ変更（「学習コース」を1項目追加するだけの案）を置き換える、より踏み込んだ再編案。まだ実装はしていない。Go/No-Go判断・命名の確定が必要。

### 10-1. 大方針

§0結論1（既存5カテゴリ・443記事には手を入れない）は維持する。今回の再編は**URLや`src/data/post/{category}/`のディレクトリ構造を変えず、ナビゲーション・トップページ・見せ方（メッセージング）だけを3本柱＋副次2種に再構成する**レイヤーの変更。既存記事のSEO評価・カテゴリ分類ルール（`category_rules.md`）は無傷のまま活きる。

### 10-2. 新3本柱・副次2種と既存資産のマッピング

| 新しい柱 | 実体（既存資産） | 性質 |
| --- | --- | --- |
| **① 20時間学習** | `course` コレクション（Week2でIT パスポート Tier1完成済み、Week3実装予定） | 体系的な学習パス（順路・進捗・章末チェック） |
...
- 「資格から探す」（`/certifications/`）は資格軸の横断入口として引き続き独立（§9-3の階層関係は変更なし）
- 「20時間学習」→ `/course/`
- 「生成AIと学ぶ」→ `/category/method/`（ラベルを「学習メソッド」から変更）
- 「資格試験の教材」→ 新設ランディング `/materials/`（後述10-4）。ドロップダウンで「クイズ・アプリ」（`/category/app/`）と「用語辞典」（`/category/theory/`）に分岐
...
1. Hero: 「20時間学習法」を第1CTA（§9-2の結論を維持）
...
### 10-4. 実装スコープ（Week3以降・§9-2を置き換え）

| # | 作業 | 影響範囲 |
| --- | --- | --- |
| 10-4-1 | `navigation.ts` のラベル・並び順変更、「資格試験の教材」ドロップダウン追加 | `src/navigation.ts` |
| 10-4-2 | `/materials/index.astro` 新設（app・theoryを横断するランディング） | 新規ファイル1点 |
| 10-4-3 | トップページの3本柱カードセクション・副次セクション追加 | `src/pages/index.astro`、新規Widgetコンポーネント1〜2点 |
| 10-4-4 | `category_rules.md`にラベル変更の対応表を追記（ディレクトリ名・`category:`値は不変である旨を明記） | `.agents/category_rules.md` |

§9-2の作業（ヘッダー「学習コース」追加・Hero CTA差し替え）は本節に統合され、Week3で1回のみ実施する。

### 10-5. 未確定事項（ユーザー確認が必要）
