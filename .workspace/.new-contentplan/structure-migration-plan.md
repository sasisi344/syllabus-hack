...
> §9-2のナビ変更（「学習コース」を1項目追加するだけの簡易案）を置き換える、より踏み込んだ再編案。**正本は `.workspace/.task/site-structure-pillars.md` へ切り出し済み**（2026-09-09、細部調整のため）。まだ実装はしていない。実装時はWeek3のタスクとして同ファイルを参照すること。
