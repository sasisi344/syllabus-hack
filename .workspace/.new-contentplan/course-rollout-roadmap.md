...
3. **確定した着手順（ユーザー指示・2026-09-09、2026-09-17更新）**: ITパスポート（`ip`、実装済み）→ 情報セキュリティマネジメント（`sg`、実装済み）→ **データマネジメント試験（`dm`）**。DM着手の方針詳細は[`dm-course/dm-course-task.md`](./dm-course/dm-course-task.md)参照
