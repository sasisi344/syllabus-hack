...
**まだ着手しない。** 今月（2026-09）はITパスポート（Tier1）の完成・公開に集中する（`new-content-plan.md`参照）。
...
3. **確定した着手順（ユーザー指示・2026-09-09）**: ITパスポート（`ip`、実装済み）→ **情報セキュリティマネジメント（`sg`）**
...
| データマネジメント試験（`dm`、新設） | 2027年夏〜秋、新設 | `syllabus-dm-2027.json`取得・構造化済み | examId未登録（`archive/completed-2026-09-09.md`参照。config.ts enumへの正式登録は現行試験終了後に見送り済み） |
| プロフェッショナルデジタルスキル試験（`pd-m`/`pd-s`、新設・3区分） | 2027年夏〜秋、新設 | `syllabus-pd-m-2027.json`・`syllabus-pd-s-2027.json`取得・構造化済み | 同上（examId未登録） |
