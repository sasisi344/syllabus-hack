...
- [ ] 章末理解度チェック問題（選択式3〜4問×章、最終章は総合復習）を作成 → **未着手**。各章ファイルの「章末チェック」に出題テーマの方向性のみコメントで記録済み。既存334問バンク（`src/data/quiz/it-passport/`）からの選定は`ip-course-curriculum.md` §5-1の章別マッピングを参照して次工程で実施（第3章は既存問題0件のため全問新規作成が必要）

## 次Weekへの引き継ぎ

- 完成した診断テスト・カリキュラムマップ・8章分の本文原稿（Tier1一式） → Week3のAstro実装で流し込む素材一式。draft: trueのまま、実装時にレビュー・trueを外す
- 章末理解度チェック問題（選択式3〜4問×章）は未作成のまま。Week3着手前、または実装と並行して仕上げる必要あり
